...
> We can also customize `LaTeX-math-list` to change the shortcuts.  
...
`section` Creates a properly formatted section. Fill in title and date.  

`set` Creates a `{...}` that is formatted for math mode.  

`*` Creates `\cdot` for multiplication.  

`<=>` Creates `\Longleftrightarrow` for iff.  

`->` Creates `\rightarrow`.  

`pd` Creates `\frac{\partial }{\partial }` for partial differentials.  

`im` Creates `\[\]` for math mode.  
